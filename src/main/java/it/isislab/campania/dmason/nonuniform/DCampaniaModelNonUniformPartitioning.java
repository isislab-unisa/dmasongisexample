...
import it.isislab.campania.dmason.nonuniform.agents.DAgentNonUniformPartitioning;
...
public class DCampaniaModelNonUniformPartitioning extends DistributedState<Double2D> {
	...
	public DCampaniaModelNonUniformPartitioning() {	}
	
	public DCampaniaModelNonUniformPartitioning(GeneralParam params,String topic)
	{
...
//		((DistributedStateConnectionFake)super.getDistributedStateConnectionJMS()).setupfakeconnection(this);
		...
		try 
		{
			...
			population = DContinuousGrid2DFactory.createDContinuous2DNonUniform(neighborhood/1.5,WIDTH, HEIGHT,this,super.AOI,TYPE.pos_j, P, MODE, "flockers", topicPrefix, true);
			init_connection();
			...
			for(MasonGeometry location :locations)
			{
				...
				if(population.verifyPosition(new Double2D(p.getX(),p.getY())))
				{
					...
					if(population.verifyPosition(new Double2D(born_road.x,born_road.y)))
					{
						...
						DAgentNonUniformPartitioning a=new DAgentNonUniformPartitioning(this, near_roads.get(random_road).getGeometry().getCoordinate(), (LineString) near_roads.get(random_road).getGeometry());
						try {
...
//	}
	...
	public static void main(String[] args) {
		doLoop(DCampaniaModelNonUniformPartitioning.class, args);
		System.exit(0);
	}
	@Override
